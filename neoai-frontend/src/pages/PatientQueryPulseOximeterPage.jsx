...
const QUERY_PAGE_STATE_KEY = "neoai-query-pulse-oximeter-page-state";
...
export function PatientQueryPulseOximeterPage() {
  ...
  const filteredExaminations = useMemo(() => {
    if (!patient || !patient.pulseOximeterExaminations) {
      return [];
    }
    ...
    return [...patient.pulseOximeterExaminations]
      ...
      .sort((left, right) => {
        ...
        } else if (sortConfig.key === "signalData") {
          comparison = left.signalData.length - right.signalData.length;
        }
      ...
      >
        <Stack spacing={2.5}>
          <Box>
            ...
            <Typography variant="h4">Search patient records (Pulse Oximeter)</Typography>
          </Box>
              ...
              <Table>
                <TableHead>
                  <TableRow >
                    ...
                    <TableCell>
                      <Button color="inherit" onClick={() => handleSort("signalData")} sx={{ px: 0, fontWeight: 700 }}>
                        Data Points
                        <Box component="span" sx={{ ml: 0.75, color: "primary.main" }}>
                          {getSortIndicator("signalData")}
                        </Box>
                      </Button>
                    </TableCell>
                  </TableRow>
                    ...
                    return (
                      <Fragment key={examination.id}>
                        ...
                        >
                          ...
                          <TableCell>{examination.signalData.length}</TableCell>
                        </TableRow>
                        <TableRow>
                            ...
                            <Collapse in={isExpanded} timeout="auto" unmountOnExit>
                              <Box sx={{ margin: 1 }}>
                                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
                                  <Typography fontWeight={700}>Examination Details</Typography>
                                  <Button
                                    component={Link}
                                    to={`/preprocessing-pulse-oximeter/${patient.id}/${examination.id}`}
                                ...
                                <Typography>Signal Data Length: {examination.signalData.length}</Typography>
                              </Box>
                            </Collapse>
                          </TableCell>
                        </TableRow>
